...
CREATE TABLE news_image_slider (id INT AUTO_INCREMENT NOT NULL, image VARCHAR(255) NOT NULL, string VARCHAR(255) NOT NULL, path VARCHAR(255) DEFAULT NULL, visible TINYINT(1) NOT NULL, PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB
ALTER TABLE news_image_slider ADD url VARCHAR(255) DEFAULT NULL, DROP string
CREATE TABLE news_Page (id INT AUTO_INCREMENT NOT NULL, title VARCHAR(255) NOT NULL, content LONGTEXT NOT NULL, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB
CREATE TABLE news_Layout (id INT AUTO_INCREMENT NOT NULL, column_one_id INT DEFAULT NULL, column_two_id INT DEFAULT NULL, column_three_id INT DEFAULT NULL, UNIQUE INDEX UNIQ_B13FADA05BAD6BDC (column_one_id), UNIQUE INDEX UNIQ_B13FADA030F18C13 (column_two_id), UNIQUE INDEX UNIQ_B13FADA09D7F31B4 (column_three_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB;
ALTER TABLE news_Layout ADD CONSTRAINT FK_B13FADA05BAD6BDC FOREIGN KEY (column_one_id) REFERENCES news_Category (id);
ALTER TABLE news_Layout ADD CONSTRAINT FK_B13FADA030F18C13 FOREIGN KEY (column_two_id) REFERENCES news_Category (id);
ALTER TABLE news_Layout ADD CONSTRAINT FK_B13FADA09D7F31B4 FOREIGN KEY (column_three_id) REFERENCES news_Category (id)
-- Valores iniciales
